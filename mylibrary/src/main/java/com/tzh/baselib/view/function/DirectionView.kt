...
import com.tzh.baselib.util.ViewMarginsUtil
                ...
                when (direction) {
                    0 -> {//
                        it.finish.let { mIt ->
                            ...
                            com.tzh.baselib.util.ViewMarginsUtil.setMarginsToPx(mIt,0,0,width,0)
                        }
                    }
                    1 -> {
                        it.finish.let { mIt ->
                            val height = mHeight
                            val width = mWidth - (mWidth * (mProgress/max)).toInt()
                            com.tzh.baselib.util.ViewMarginsUtil.setMarginsToPx(mIt,width,0,0,0)
                        }
                    }
                    2 -> {
                        it.finish.let { mIt ->
                            ...
                            val width = mWidth
                            com.tzh.baselib.util.ViewMarginsUtil.setMarginsToPx(mIt,0,0,0,height)
                        }
                    }
                    3 -> {
                        it.finish.let { mIt ->
                            val height = mHeight- (mHeight * (mProgress/max)).toInt()
                            val width = mWidth
                            com.tzh.baselib.util.ViewMarginsUtil.setMarginsToPx(mIt,0,height,0,0)
                        }
                    }
                    else->{
