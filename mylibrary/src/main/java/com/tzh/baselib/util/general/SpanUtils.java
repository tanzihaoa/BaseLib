...
public class SpanUtils {
     ...
     * @return the single {@link SpanUtils} instance
     */
    public SpanUtils setUnderline(boolean isShow) {
        this.isUnderline = isShow;
        return this;
    }
    ...
    private void updateImage() {
        ...
        int end = mBuilder.length();
        if (imageBitmap != null) {
            mBuilder.setSpan(new CustomImageSpan(mTextView.getContext().getApplicationContext(),imageBitmap, alignImage), start, end, flag);
        } else if (imageDrawable != null) {
            mBuilder.setSpan(new CustomImageSpan(mTextView.getContext().getApplicationContext(),imageDrawable, alignImage), start, end, flag);
        } else if (imageUri != null) {
            mBuilder.setSpan(new CustomImageSpan(mTextView.getContext().getApplicationContext(),imageUri, alignImage), start, end, flag);
        } else if (imageResourceId != -1) {
            mBuilder.setSpan(new CustomImageSpan(mTextView.getContext().getApplicationContext(),imageResourceId, alignImage), start, end, flag);
        }
    }
